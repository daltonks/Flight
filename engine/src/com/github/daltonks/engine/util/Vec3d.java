...
public class Vec3d implements Positionable {
    ...
    public Vec3d multMatrix(float[] matrix) {
        double x = this.x;
        double y = this.y;
        double z = this.z;
    ...
    /**
        @return the 'w' value
     */
    public float multMatrix(float[] matrix, float w) {
        float[] loc = Pools.getFloat4();
        get(loc);
        loc[3] = w;
        float[] result = Pools.getFloat4();
        Matrix.multiplyMV(result, 0, matrix, 0, loc, 0);
        set(result);
        Pools.recycleFloat4(loc);
        Pools.recycleFloat4(result);
        return result[3];
    }
