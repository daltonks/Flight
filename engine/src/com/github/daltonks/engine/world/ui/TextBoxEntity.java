...
public class TextBoxEntity extends UIEntity {
    ...
    public class TextBoxDrawableComponent extends DrawableComponent {
        ...
        public void updateLetterLocations() {
            ...
            for(int i = 0; i < letters.size(); i++) {
                Letter letter = letters.get(i);
                tempOffsetVec[0] = letter.offsetX;
                tempOffsetVec[1] = letter.offsetY;
                float[] rotMatrix = transform.getRotationMatrix();
                Matrix.multiplyMV(tempLoc, 0, rotMatrix, 0, tempOffsetVec, 0);
                Vec3d loc = transform.getLocation();
                letter.loc.set(loc.x() + tempLoc[0], loc.y() + tempLoc[1], loc.z() + tempLoc[2]);
            }
        }
    ...
    private static float[] tempOffsetVec = {0, 0, 0, 1};
    private static float[] tempLoc = new float[4];
